...
from payments.mpesa_vendor import VendorMpesaService
...
# Transaction helpers
# -------------------------
def create_transaction(phone, plan_id, vendor_id, mikrotik_id, mikrotik_identity_name, amount, hotspot_data=None):

    tx_uuid = str(uuid.uuid4())
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("""
        INSERT INTO user_transactions (transaction_uuid, client_phone, plan_id, vendor_id, mikrotik_id, amount, mikrotik_identity_name, status, mac, ip, username, sessionid, created_at, link_login)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s)
    """, (tx_uuid, phone, plan_id, vendor_id, mikrotik_id, amount, mikrotik_identity_name, 'pending',
...
##_______LOAD VENDOR BY ID__________________##
def load_vendor_mpesa(vendor_id):
    conn = get_connection()
    cursor = conn.cursor(dictionary=True)
    ...
    if not row:
        raise Exception("Vendor has no active M-PESA configuration")

    return VendorMpesaService(
        business_shortcode=row["business_shortcode"],
        consumer_key=row["consumer_key"],
        consumer_secret=row["consumer_secret"],
        passkey=row["passkey"],
        environment=row["environment"],
        transaction_updater=update_transaction_with_stk_response
    )
...
def payment():
    ...
    mikrotik_identity_name = hotspot_data.get('identity', '')
    ...
    try:
        ...
        # user exists and has an active session
        if user and user.get("session_status") == "active":

            # SAME vendor → block purchase
            if user.get("mikrotik_identity_name") == mikrotik_identity_name:
                return jsonify({
                    "status": "blocked",
                    "message": "Your purchased session is still active on this vendor."
                }), 200

            # DIFFERENT vendor → allow switching
            else:
                return jsonify({
            "status": "switch_allowed",
            "message": "Active session detected on another vendor. You may proceed to purchase."
        }), 200

        # 1. Resolve router → vendor
        cur.execute("""
            SELECT id, vendor_id
        ...
        if not router:
            return {"error": "You are logging\
                    in from Unregistered node \
                    identity"}, 403

        vendor_id = router.get("vendor_id")
        mikrotik_id = router.get("id")
            
        # Create transaction record
        tx_uuid = create_transaction(phone, plan_id, vendor_id, \
                                     mikrotik_id, mikrotik_identity_name,\
                                     amount, hotspot_data)
        logging.info(f'TX DATA {tx_uuid} ')

         # 3. Load vendor M-PESA service
        mpesa = load_vendor_mpesa(vendor_id)
        ...
        response = mpesa.stk_push(
            ...
            phone=phone,
            amount=amount,
            callback_url=f"{callback_url}?tx={tx_uuid}",
            account_reference=mikrotik_identity_name,
        ...
        if result_code == 0:
            ...
            if tx:
                ...
                try:
                    ...
                    if user:
                        cur.execute("""
                                    UPDATE hotspot_users SET transaction_uuid=%s, mac=%s, mikrotik_profile=%s, expires_at=%s, \
                                    client_ip=%s, link_login=%s, code_6char=%s,mikrotik_identity_name=%s,session_status=%s,mikrotik_id=%s
                                    WHERE username=%s
                                """, (transaction_uuid, tx.get('mac'), plan.get('mikrotik_profile') if plan else '', expires_at, \
                                     tx.get('ip'), tx.get('link_login'), password, tx.get('mikrotik_identity_name'), 'active', tx.get('mikrotik_id'), username))
                        conn.commit()
                        ...
                        cur.execute("""
                            INSERT INTO hotspot_users (transaction_uuid, mac, username, mikrotik_profile, expires_at, client_ip, \
                                    link_login, code_6char, mikrotik_identity_name,mikrotik_id)
                                    VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                        """, (transaction_uuid, tx.get('mac'), username, plan.get('mikrotik_profile') if plan else '', expires_at,\
                             tx.get('ip'), tx.get('link_login'), password, tx.get('mikroti_identity_name'), tx.get('mikrotik_id')))
                        conn.commit()
...
def reconnect(username):
    ...
    mikrotik_identity_name = data.get('mikrotik_identity_name')
    ...
    try:
        ...
        if mikrotik_identity_name != user.get('mikrotik_identity_name'):
            html =  """
                    <button><a href='http://levine.net'>confirm switching between vendors</button>
                    """
            return make_response(html, {"Content-Type":"text/html"})
